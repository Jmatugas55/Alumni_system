...
useEffect(() => {
  const fetchData = async () => {
    await fetchProfile();
    await fetchAlumniList(); // 👈 replaced fetchJobs
...
const fetchAlumniList = async () => {
  try {
    ...
    const alumniUsers = res.data.filter(user => user.role === "alumni");
    // Optionally limit displayed users to first 5
    setAlumniList(alumniUsers.slice(0, 5));
  } catch (error) {
                  ...
                  <ul className="user_dashboard_list">
                    {alumniList.length === 0 ? (
                      ...
                      alumniList.map((alumni) => (
                        <li key={alumni.user_id} className="alumni_item">
                          {/* Alumni Photo */}
                          <img
                          ...
                          <div className="alumni_info">
                            ...
                            <span>{alumni.course} | Class of {alumni.graduation_year}</span><br />
